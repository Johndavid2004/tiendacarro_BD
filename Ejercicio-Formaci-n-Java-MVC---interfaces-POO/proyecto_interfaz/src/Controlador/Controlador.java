...
public class Controlador {
    ...
    private void registrarVehiculo() {
        ...
        if (marca.isEmpty() || modelo.isEmpty() || placa.isEmpty()) {
            JOptionPane.showMessageDialog(formVehiculo, "Por favor, complete todos los campos.", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }

        Vehiculo vehiculo;
        if (tipo.equals("AUTOMOVIL")) {
            vehiculo = new Coche(marca, modelo, placa, 4);
        } else {
            vehiculo = new Moto(marca, modelo, placa, 150);
        }
